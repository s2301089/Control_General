...
````admonish warning "順序"
C言語は、関数の中身は上の文から順番に実行されます。`return`文を実行すると、関数内のその後の文は実行されません。  
...
````admonish info "if文"
「もし 〇〇 ならば これを実行する」を表現する文です。指定した条件をが真か偽かを判定し、処理を2つに分岐されることができる。  
...
処理が1行であり、偽の場合の処理がない場合は文を`{}`で囲む必要はなく、`else`以降も不要です。  
...
それぞれ、配列dは`d[0]~d[9]`まで、配列fは`f[0]~[3]`の変数が宣言されます。配列番号(定数式)は`0`から始まるため、`0~n-1`までの`n`個になります。  
````

````admonish example "配列へのアクセス"
配列へのアクセスは変数と代わりありません。しかし、要素番号を指定する必要があります。要素番号は整数であれば変数や式も使用できます。  
