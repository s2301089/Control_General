...
void loop() {
  ...
  if(DS5.connected()) { //DualShock4接続確認
    
    data[LX]  = DS5.getAnalogHat(LeftHatX);  //LスティックX軸
    data[LY]  = DS5.getAnalogHat(LeftHatY);  //LスティックY軸
    data[RX]  = DS5.getAnalogHat(RightHatX); //RスティックX軸
    data[RY]  = DS5.getAnalogHat(RightHatY); //RスティックY軸
    data[L_2] = DS5.getAnalogButton(L2);     //L2ボタン
    data[R_2] = DS5.getAnalogButton(R2);     //R2ボタン

    //ButtonAssign1 値検出
    if(DS5.getButtonPress(TRIANGLE))  data[BA1] |= 0x01; //三角ボタン
    if(DS5.getButtonPress(CIRCLE))    data[BA1] |= 0x02; //丸ボタン
    if(DS5.getButtonPress(CROSS))     data[BA1] |= 0x04; //バツボタン
    if(DS5.getButtonPress(SQUARE))    data[BA1] |= 0x08; //四角ボタン
    if(DS5.getButtonPress(UP))        data[BA1] |= 0x10; //十字キー上
    if(DS5.getButtonPress(RIGHT))     data[BA1] |= 0x20; //十字キー右
    if(DS5.getButtonPress(DOWN))      data[BA1] |= 0x40; //十字キー下
    if(DS5.getButtonPress(LEFT))      data[BA1] |= 0x80; //十字キー左

    //ButtonAssign2 値検出
    if(DS5.getButtonPress(L1))        data[BA2] |= 0x01; //L1
    if(DS5.getButtonPress(L3))        data[BA2] |= 0x02; //L3(左スティック押しこみ)
    if(DS5.getButtonPress(R1))        data[BA2] |= 0x04; //R1
    if(DS5.getButtonPress(R3))        data[BA2] |= 0x08; //R3(右スティック押しこみ)
  if(DS5.getButtonPress(CREATE))      data[BA2] |= 0x10; //SHAREボタン
    if(DS5.getButtonPress(OPTIONS))   data[BA2] |= 0x20; //OPTIONSボタン
    if(DS5.getButtonPress(PS))        data[BA2] |= 0x40; //PSボタン
    if(DS5.getButtonPress(TOUCHPAD))  data[BA2] |= 0x80; //TOUCHPADボタン
    ...
    if(data[BA2] & 0x10) Serial.print("CREATE ");
