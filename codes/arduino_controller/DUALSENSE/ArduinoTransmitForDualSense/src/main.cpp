...
void loop() {
  ...
  if(DUALSENSE.connected()){
    // アナログ値の検出
    data[LX]  = DUALSENSE.getAnalogHat(LeftHatX);
    data[LY]  = DUALSENSE.getAnalogHat(LeftHatY);
    data[RX]  = DUALSENSE.getAnalogHat(RightHatX);
    data[RY]  = DUALSENSE.getAnalogHat(RightHatY);
    data[L2S] = DUALSENSE.getAnalogButton(L2);
    data[R2S] = DUALSENSE.getAnalogButton(R2);

    // ボタンセット1の検出
    if(DUALSENSE.getButtonPress(TRIANGLE))  data[BS1] |= 0x01;
    if(DUALSENSE.getButtonPress(CIRCLE))    data[BS1] |= 0x02;
    if(DUALSENSE.getButtonPress(CROSS))     data[BS1] |= 0x04;
    if(DUALSENSE.getButtonPress(SQUARE))    data[BS1] |= 0x08;
    if(DUALSENSE.getButtonPress(UP))        data[BS1] |= 0x10;
    if(DUALSENSE.getButtonPress(RIGHT))     data[BS1] |= 0x20;
    if(DUALSENSE.getButtonPress(DOWN))      data[BS1] |= 0x40;
    if(DUALSENSE.getButtonPress(LEFT))      data[BS1] |= 0x80;

    // ボタンセット2の検出
    if(DUALSENSE.getButtonPress(L1))        data[BS2] |= 0x01;
    if(DUALSENSE.getButtonPress(L3))        data[BS2] |= 0x02;
    if(DUALSENSE.getButtonPress(R1))        data[BS2] |= 0x04;
    if(DUALSENSE.getButtonPress(R3))        data[BS2] |= 0x08;
    if(DUALSENSE.getButtonPress(CREATE))    data[BS2] |= 0x10;
    if(DUALSENSE.getButtonPress(OPTIONS))   data[BS2] |= 0x20;
    if(DUALSENSE.getButtonPress(PS))        data[BS2] |= 0x40;
    if(DUALSENSE.getButtonPress(TOUCHPAD))  data[BS2] |= 0x80;
