...
void loop() {
  ...
  if(DUALSHOCK3.PS3Connected || DUALSHOCK3.PS3NavigationConnected){
    // アナログ値の検出
    data[LX]  = DUALSHOCK3.getAnalogHat(LeftHatX);
    data[LY]  = DUALSHOCK3.getAnalogHat(LeftHatY);
    data[RX]  = DUALSHOCK3.getAnalogHat(RightHatX);
    data[RY]  = DUALSHOCK3.getAnalogHat(RightHatY);
    data[L2S] = DUALSHOCK3.getAnalogButton(L2);
    data[R2S] = DUALSHOCK3.getAnalogButton(R2);

    // ボタンセット1の検出
    if(DUALSHOCK3.getButtonPress(TRIANGLE))  data[BS1] |= 0x01;
    if(DUALSHOCK3.getButtonPress(CIRCLE))    data[BS1] |= 0x02;
    if(DUALSHOCK3.getButtonPress(CROSS))     data[BS1] |= 0x04;
    if(DUALSHOCK3.getButtonPress(SQUARE))    data[BS1] |= 0x08;
    if(DUALSHOCK3.getButtonPress(UP))        data[BS1] |= 0x10;
    if(DUALSHOCK3.getButtonPress(RIGHT))     data[BS1] |= 0x20;
    if(DUALSHOCK3.getButtonPress(DOWN))      data[BS1] |= 0x40;
    if(DUALSHOCK3.getButtonPress(LEFT))      data[BS1] |= 0x80;

    // ボタンセット2の検出
    if(DUALSHOCK3.getButtonPress(L1))        data[BS2] |= 0x01;
    if(DUALSHOCK3.getButtonPress(L3))        data[BS2] |= 0x02;
    if(DUALSHOCK3.getButtonPress(R1))        data[BS2] |= 0x04;
    if(DUALSHOCK3.getButtonPress(R3))        data[BS2] |= 0x08;
    if(DUALSHOCK3.getButtonPress(SELECT))    data[BS2] |= 0x10;
    if(DUALSHOCK3.getButtonPress(START))     data[BS2] |= 0x20;
    if(DUALSHOCK3.getButtonPress(PS))        data[BS2] |= 0x40;
    ...
    if(data[BS2] & 0x10)  Serial.print("SELECT ");
    if(data[BS2] & 0x20)  Serial.print("START ");
